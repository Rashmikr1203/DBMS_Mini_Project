...
--Update Operations
ALTER TABLE Book MODIFY availability tinyint(1) AFTER book_id;
ALTER TABLE User MODIFY role VARCHAR(50) DEFAULT 'Student' NOT NULL;
ALTER TABLE Staff ADD CONSTRAINT max_salary CHECK(salary <= 100000.00);
RENAME TABLE Research_Paper TO Resaerch_publication;
ALTER TABLE Staff MODIFY library_id AFTER name;
...
-- Step 1: Drop foreign key constraints in the Transaction and fine_payment tables
ALTER TABLE Transaction DROP FOREIGN KEY transaction_ibfk_1;
ALTER TABLE Transaction DROP FOREIGN KEY transaction_ibfk_2;

-- Drop foreign key constraint in the fine_payment table
ALTER TABLE fine_payment DROP FOREIGN KEY fine_payment_ibfk_1;

-- Step 2: Modify transaction_id column in the Transaction table to AUTO_INCREMENT
ALTER TABLE Transaction MODIFY COLUMN transaction_id INT NOT NULL AUTO_INCREMENT;

-- Step 3: Set AUTO_INCREMENT starting value to 1
ALTER TABLE Transaction AUTO_INCREMENT = 1;

-- Step 4: Re-add foreign key constraints to the Transaction table
ALTER TABLE Transaction ADD CONSTRAINT transaction_ibfk_1 FOREIGN KEY (book_id) REFERENCES book(book_id);
ALTER TABLE Transaction ADD CONSTRAINT transaction_ibfk_2 FOREIGN KEY (user_id) REFERENCES user(user_id);

-- Step 5: Re-add foreign key constraint in the fine_payment table
ALTER TABLE fine_payment
ADD CONSTRAINT fine_payment_ibfk_1 FOREIGN KEY (transaction_id) REFERENCES Transaction(transaction_id);


--Delete Operations-gives errors
-- Drop and re-add foreign key for the `Staff` table
ALTER TABLE `Staff`
...
-- Drop and re-add foreign key for the `fine_payment` table
ALTER TABLE `fine_payment`
DROP FOREIGN KEY `fine_payment_ibfk_1`;

ALTER TABLE `fine_payment`
ADD CONSTRAINT `fine_payment_ibfk_1`
FOREIGN KEY (`transaction_id`) REFERENCES `transaction` (`transaction_id`)
ON DELETE CASCADE;
...
-- Grant read and write permissions on the `book` table
GRANT SELECT, INSERT, UPDATE, DELETE ON library_mngmt_system_final.book TO UserRole;

-- Grant read-only permissions on all other tables
GRANT SELECT ON library_mngmt_system_final.acquisition TO UserRole;
GRANT SELECT ON library_mngmt_system_final.fine_payment TO UserRole;
GRANT SELECT ON library_mngmt_system_final.library TO UserRole;
GRANT SELECT ON library_mngmt_system_final.notification TO UserRole;
GRANT SELECT ON library_mngmt_system_final.research_publication TO UserRole;
GRANT SELECT ON library_mngmt_system_final.staff TO UserRole;
GRANT SELECT ON library_mngmt_system_final.transaction TO UserRole;
GRANT SELECT ON library_mngmt_system_final.user TO UserRole;

-- Grant full read and write permissions on all tables to AdminRole
GRANT ALL PRIVILEGES ON library_mngmt_system_final.* TO AdminRole;

-- Create users with host as '%' and the corresponding passwords
CREATE USER 'Alice Johnson'@'%' IDENTIFIED BY '';
CREATE USER 'Bob Smith'@'%' IDENTIFIED BY '';
CREATE USER 'Daisy Chen'@'%' IDENTIFIED BY '';
CREATE USER 'Edward Kim'@'%' IDENTIFIED BY '';
CREATE USER 'Fiona White'@'%' IDENTIFIED BY '';
CREATE USER 'George Martin'@'%' IDENTIFIED BY '';
CREATE USER 'Hannah Brown'@'%' IDENTIFIED BY '';
CREATE USER 'Ian Green'@'%' IDENTIFIED BY '';
CREATE USER 'Jack Black'@'%' IDENTIFIED BY '';
CREATE USER 'Liam Clark'@'%' IDENTIFIED BY '';
CREATE USER 'Mia Lewis'@'%' IDENTIFIED BY '';
CREATE USER 'Nina Wilson'@'%' IDENTIFIED BY '';
CREATE USER 'Oscar Brown'@'%' IDENTIFIED BY '';
CREATE USER 'Paul Adams'@'%' IDENTIFIED BY '';
CREATE USER 'Quincy Hall'@'%' IDENTIFIED BY '';
CREATE USER 'Rachel Evans'@'%' IDENTIFIED BY '';
CREATE USER 'Steve Price'@'%' IDENTIFIED BY '';
CREATE USER 'Tina Turner'@'%' IDENTIFIED BY '';
CREATE USER 'rashmi'@'%' IDENTIFIED BY '';
CREATE USER 'rashmi1'@'%' IDENTIFIED BY 'rashmi1';
CREATE USER 'rashmi'@'%' IDENTIFIED BY 'rashmi2';
CREATE USER 'pranjal'@'%' IDENTIFIED BY 'pranjal';
CREATE USER 'Nishita'@'%' IDENTIFIED BY 'ep.nish1';
CREATE USER 'Neha'@'%' IDENTIFIED BY 'neha';
CREATE USER 'neha'@'%' IDENTIFIED BY 'neha';

-- Assign UserRole to each user
GRANT UserRole TO 'Alice Johnson'@'%', 'Bob Smith'@'%', 'Daisy Chen'@'%', 
                 'Edward Kim'@'%', 'Fiona White'@'%', 'George Martin'@'%', 
                 'Hannah Brown'@'%', 'Ian Green'@'%', 'Jack Black'@'%', 
                 'Liam Clark'@'%', 'Mia Lewis'@'%', 'Nina Wilson'@'%', 
                 'Oscar Brown'@'%', 'Paul Adams'@'%', 'Quincy Hall'@'%', 
                 'Rachel Evans'@'%', 'Steve Price'@'%', 'Tina Turner'@'%', 
                 'rashmi'@'%', 'rashmi1'@'%', 'rashmi'@'%', 'pranjal'@'%', 
                 'pranjal'@'%', 'Nishita'@'%', 'Neha'@'%', 'neha'@'%';
    
-- Create users for Staff members with host as '%' and the corresponding passwords
CREATE USER 'Sarah Collins'@'%' IDENTIFIED BY '';
CREATE USER 'Tom Harris'@'%' IDENTIFIED BY '';
CREATE USER 'Lucy Fisher'@'%' IDENTIFIED BY '';
CREATE USER 'Max Roberts'@'%' IDENTIFIED BY '';
CREATE USER 'Linda Walker'@'%' IDENTIFIED BY '';
CREATE USER 'Chris Johnson'@'%' IDENTIFIED BY '';
CREATE USER 'Angela Smith'@'%' IDENTIFIED BY '';
CREATE USER 'John Baker'@'%' IDENTIFIED BY '';
CREATE USER 'Emily Young'@'%' IDENTIFIED BY '';
CREATE USER 'Dylan Hill'@'%' IDENTIFIED BY '';
CREATE USER 'Chloe Taylor'@'%' IDENTIFIED BY '';
CREATE USER 'Jason King'@'%' IDENTIFIED BY '';
CREATE USER 'Megan Wright'@'%' IDENTIFIED BY '';
CREATE USER 'Nathan Scott'@'%' IDENTIFIED BY '';
CREATE USER 'Olivia Evans'@'%' IDENTIFIED BY '';
CREATE USER 'Oscar Turner'@'%' IDENTIFIED BY '';
CREATE USER 'Sophia Morgan'@'%' IDENTIFIED BY '';
CREATE USER 'Liam Bell'@'%' IDENTIFIED BY '';
CREATE USER 'Grace Allen'@'%' IDENTIFIED BY '';
CREATE USER 'Evan Mitchell'@'%' IDENTIFIED BY '';
CREATE USER 'Nishita'@'%' IDENTIFIED BY 'ep.nish';
CREATE USER 'vasu'@'%' IDENTIFIED BY 'vasu';
CREATE USER 'ramesh'@'%' IDENTIFIED BY 'ramesh';

-- Assign AdminRole to each staff member
GRANT AdminRole TO 'Sarah Collins'@'%', 'Tom Harris'@'%', 'Lucy Fisher'@'%', 
                   'Max Roberts'@'%', 'Linda Walker'@'%', 'Chris Johnson'@'%', 
                   'Angela Smith'@'%', 'John Baker'@'%', 'Emily Young'@'%', 
                   'Dylan Hill'@'%', 'Chloe Taylor'@'%', 'Jason King'@'%', 
                   'Megan Wright'@'%', 'Nathan Scott'@'%', 'Olivia Evans'@'%', 
                   'Oscar Turner'@'%', 'Sophia Morgan'@'%', 'Liam Bell'@'%', 
                   'Grace Allen'@'%', 'Evan Mitchell'@'%', 'Nishita'@'%', 
                   'vasu'@'%', 'ramesh'@'%';

--Procedure
DELIMITER //

CREATE PROCEDURE ReturnBook(IN transId INT, IN retDate DATE)
BEGIN
    DECLARE lateFee DECIMAL(10, 2);

    -- Check if transaction exists
    IF (SELECT COUNT(*) FROM Transaction WHERE transaction_id = transId) = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Transaction ID not found';
    END IF;

    -- Calculate late fee using the CalculateLateFee function
    SET lateFee = CalculateLateFee((SELECT date_borrowed FROM Transaction WHERE transaction_id = transId), retDate);

    -- Update transaction with return date and calculated late fee
    UPDATE Transaction
    SET return_date = retDate,
        late_fees = lateFee
    ...
    WHERE book_id = (SELECT book_id FROM Transaction WHERE transaction_id = transId);

    -- Delete the transaction from the Transaction table
    DELETE FROM Transaction WHERE transaction_id = transId;

    -- Return result message and late fee
    SELECT 'Book returned successfully and transaction deleted!' AS message, lateFee AS lateFee;
END //

DELIMITER ;

--Log Report
CREATE TABLE my_book_log (
...
ALTER TABLE my_book_log CHANGE COLUMN publisher_name publisher VARCHAR(40);

-- TRIGGERS
DELIMITER //

CREATE TRIGGER my_book_after_insert
AFTER INSERT ON book
FOR EACH ROW
BEGIN
    INSERT INTO my_book_log (action_type, book_id, title, publisher)
    VALUES ('INSERT', NEW.book_id, NEW.title, NEW.publisher);
END //

CREATE TRIGGER my_book_after_update
AFTER UPDATE ON book
FOR EACH ROW
BEGIN
    INSERT INTO my_book_log (action_type, book_id, title, publisher)
    VALUES ('UPDATE', NEW.book_id, NEW.title, NEW.publisher);
END //

CREATE TRIGGER my_book_after_delete
AFTER DELETE ON book
FOR EACH ROW
BEGIN
    INSERT INTO my_book_log (action_type, book_id, title, publisher)
    VALUES ('DELETE', OLD.book_id, OLD.title, OLD.publisher);
END //

-- Trigger to automatically mark a book as available when it is returned
